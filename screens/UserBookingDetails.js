...
const UserBookingDetails = ({ route, navigation }) => {
  ...
  const handleQRScan = () => {
    if (isCharging) {
      ...
      Alert.alert(
        "Charging Started",
        "Your charging session has begun successfully!",
        [{ text: "OK", onPress: () => setShowQRCode(false) }]
      );
    }
  };
...
Booking ID: ${booking.bookingId}
      ...
      await Share.share({
        ...
        title: "EV Charging Booking Details",
      });
    } catch (error) {
  ...
  const getRouteFromGoogleMaps = async (start, end) => {
    try {
      ...
      if (data.routes && data.routes[0]) {
        ...
        const distanceInMeters = leg.distance; // Distance in meters
        console.log(`Distance: ${distanceInMeters} meters`); // Log distance

        // Calculate duration based on average speed (e.g., 30 km/h)
        const averageSpeedKmh = 30; // Set your average speed here
        const durationInSeconds =
          distanceInMeters / ((averageSpeedKmh * 1000) / 3600); // Convert speed to m/s
        console.log(`Calculated Duration: ${durationInSeconds} seconds`); // Log calculated duration

        setRouteCoordinates(decodePolyline(data.routes[0].geometry));
        setDistance(`${(distanceInMeters / 1000).toFixed(1)} km`); // Convert to kilometers
        setDuration(`${Math.round(durationInSeconds / 60)} mins`); // Convert seconds to minutes
      } else {
          ...
          <ScrollView>
            ...
            <View style={styles.infoSection}>
              ...
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{booking.duration} mins</Text>
              </View>
            </View>
      ...
      <View style={styles.modalContainer}>
        <BlurView intensity={100} style={styles.modalContent}>
          <Text style={styles.modalTitle}>
            {isCharging ? "Charging in Progress" : "Scan to Start Charging"}
          </Text>
      ...
      >
        ...
        >
          <View style={styles.stationMarker}>
            <MaterialCommunityIcons name="ev-station" size={34} color="green" />
          </View>
        </Marker>
